...
export default async function LoginPage({ searchParams }: LoginPageProps) {
  ...
  return (
    <main className="mx-auto flex min-h-screen w-full max-w-md flex-col justify-center gap-4 px-6">
      ...
      <p className="text-sm text-zinc-600">Use your account to access your learning dashboard.</p>

      {error ? <p className="rounded-md bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p> : null}

      <form action={loginAction} className="flex flex-col gap-3 rounded-xl border border-zinc-200 p-4">
